...
var LEGENDS_FILE = path.join('cards', 'legends.js');
var MIN_LEGENDS = 30;

function champOf(o) {
  var g = o.g || [];
  return g.length ? String(g[g.length - 1]) : '';
}

function titleOf(o) {
  return String(o.n || '').replace(/\s*-\s*Starter$/, '').trim();
}

function priorLegendNames() {
  var names = [];
  var txt = '';
  try { txt = fs.readFileSync(LEGENDS_FILE, 'utf8'); } catch (e) { return names; }
  var re = /"n":"((?:[^"\\]|\\.)*)"/g;
  var m = re.exec(txt);
  while (m) { names.push(JSON.parse('"' + m[1] + '"')); m = re.exec(txt); }
  return names;
}
...
function buildLegends(list, setOrder) {
  ...
  for (i = 0; i < list.length; i++) {
    o = list[i];
    if (o.t !== 'legend') { continue; }
    ch = champOf(o);
    ti = titleOf(o);
    if (!ch) { throw new Error('legend with no champion tag: ' + (o.c || o.n)); }
    if (!ti) { throw new Error('legend with no title: ' + (o.c || '?')); }
    key = ch + '|' + ti;
    if (!groups[key]) {
      groups[key] = { n: ch + ', ' + ti, d: (o.d || []).slice(), v: [] };
      order.push(key);
    }
    g = groups[key];
  ...
  var prints = 0;
  for (i = 0; i < out.length; i++) { prints += out[i].v.length; }
  var before = priorLegendNames();
  var now = [];
  for (i = 0; i < out.length; i++) { now.push(out[i].n); }
  var added = [];
  var removed = [];
  for (i = 0; i < now.length; i++) { if (before.indexOf(now[i]) < 0) { added.push(now[i]); } }
  for (i = 0; i < before.length; i++) { if (now.indexOf(before[i]) < 0) { removed.push(before[i]); } }
  var lines = [];
  lines.push('/* GENERATED by fetch-cards.js -- do not edit by hand. */');
  lines.push('/* ' + out.length + ' legends, ' + prints + ' printings. */');
  lines.push('/* n display name, d domains, v printings (c code, s set, h art hash). */');
  lines.push('var VIR_LEGENDS = [');
  for (i = 0; i < out.length; i++) { lines.push(JSON.stringify(out[i]) + ','); }
  lines.push('];');
  fs.writeFileSync(LEGENDS_FILE, lines.join('\n') + '\n', 'utf8');
  return { count: out.length, printings: prints, added: added, removed: removed, warn: warn };
}
